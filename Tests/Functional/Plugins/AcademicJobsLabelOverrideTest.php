 ...
 * this extension render, on every supported core version: a label comes from
 * `locallang.xlf`, an override of the extension (`plugin.tx_academicjobs`) replaces it, and
 * an override of the plugin (`plugin.tx_academicjobs_<plugin>`) replaces both.
 *
 * Up to TYPO3 v13 the core builds the TypoScript path from the extension name a translation
 * passes, only lowercased, so a name with underscores read `plugin.tx_academic_jobs`
 ...
 * translation of its file in the request would pass on v13 even if its own call lost the
 * name again, and TYPO3 v14 strips the underscores and falls back to the name of the plugin
