...
namespace FGTCLB\AcademicJobs\Loader;
...
class AcademicJobsSettingsLoader
{
    ...
    public function loadUncached(): array
    {
        ...
        foreach ($this->packageManager->getActivePackages() as $package) {
            $settingsFile = $package->getPackagePath() . 'Configuration/AcademicJobs/Settings.yaml';
    ...
    {
        $settings = $this->cache->require($this->academicJobsSettingsIdentifier());
        if (!is_array($settings)) {
            return null;
        }
        return $settings;
    ...
    {
        $this->cache->set($this->academicJobsSettingsIdentifier(), 'return ' . var_export($settings, true) . ';');
    }

    /**
     * @return non-empty-string string
     */
    protected function academicJobsSettingsIdentifier(): string
    {
        return 'AcademicJobs_Settings';
    }
}
