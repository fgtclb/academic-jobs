.. _feature-ace-250-academic-jobs:
...
* **Jobs List** (:php:`academicjobs_list`)

When the option is enabled, the frontend job listing includes hidden
...
Editors can now opt in per plugin instance to display hidden jobs in the
frontend, for example to preview upcoming or intentionally hidden
...
All installations using the `EXT:academic_jobs` extension starting with
version 2.4. No action is required for existing installations.

.. index:: Backend, Frontend, ext:academic_jobs
