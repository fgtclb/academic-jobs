...
..  note::
    ...
    ships are never read, and the static templates plus the page TSconfig files
...
:typoscript:`plugin.tx_academicjobs`. That block is shipped once, in
:file:`Configuration/TypoScript/`, and every component includes it. Which
...
:guilabel:`EXT:academic_jobs` hides all three of its content elements for the
...
..  list-table::
    ...
    *   -   :guilabel:`Academic Jobs: Shared plugin settings (academic_jobs)`
        -   The shared :typoscript:`plugin.tx_academicjobs` block on its own.
